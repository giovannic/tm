...
function makePieChart(containerDiv, values, x, y, radius) {
	...
	for ( var key in values) {
		...
		drawSegment(theSvg, x, y, segmentPositions, radius, makeRandomColour(), key, percent);
...
function percentToPosition(startPercent, endPercent, radius) {
	...
	result.labelX = (result.startX + result.endX) / 2;
	result.labelY = (result.startY + result.endY) / 2;

	return result;	
}

function drawSegment(theSvg, x, y, segPositions, radius, colour, text, percent) {
	...
	"L " + segPositions.startX + " " + segPositions.startY + " " + // Draw a line to the start of the arc
		...
		" 0 0 0 " + // no rotation, large arc, no sweep
	...
	var labelx = segPositions.labelX - (width/2) + 8;
	var labely = segPositions.labelY - (height/2);
	var percenty = segPositions.labelY + (height/2); 
	...
	percentLbl.setAttribute("x", segPositions.labelX);
